...
export function ChatMessages({ message, selectedChat }: { message: Message; selectedChat: Chat }) {
  ...
  return (
    <div
      key={message?.id}
      className={`flex items-start ${isActiveUser ? 'justify-end' : 'justify-start'}`}
    >
      <div
        className={`flex max-w-[70%] items-center gap-2 rounded-xl p-4 ${
          isActiveUser ? 'bg-primary text-primary-foreground' : 'bg-background'
        }`}
      >
        <Avatar>
          <AvatarFallback className='border border-primary text-primary'>
            {selectedChat?.participants.find((participant) => participant.id)?.name.charAt(0)}
          </AvatarFallback>
        </Avatar>
        <div className='flex flex-col'>
          ...
          <p
            className={`mt-1 text-xs ${
              isActiveUser ? 'text-primary-foreground/70' : 'text-muted-foreground'
            }`}
          >
